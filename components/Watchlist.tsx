...
export function Watchlist({ profileUid, profileUsername }: { profileUid: string, profileUsername: string }) {
  ...
  useEffect(() => {
    ...
    const backfillMetadata = async () => {
      // Find items missing releaseYear or rating (either undefined or null)
      const itemsToUpdate = items.filter(item => 
        (item.releaseYear === undefined || item.releaseYear === null || 
        ...
        try {
          ...
          if (item.type === 'books' || (item.type as string) === 'comics') {
            ...
            if (options.length > 0) {
              ...
              if (option.imageUrl) {
                if (option.source === 'Open Library') {
                  imgUrl = option.imageUrl.replace('-S.jpg', '-M.jpg');
                } else if (option.source === 'MangaDex') {
                  imgUrl = option.imageUrl.replace('.256.jpg', '');
                }
              }
              
              metadata = {
                imageUrl: imgUrl,
                releaseYear: option.releaseYear || item.releaseYear || null,
                author: option.author || item.author || null,
                rating: option.rating !== undefined ? option.rating : (item.rating || null)
              };
            }
          } else if (item.type === 'anime') {
            const options = await searchAniListOptions(item.name, 'ANIME');
            if (options.length > 0) {
              const option = options[0];
              foundMatch = true;
              metadata = {
            ...
            if (options.length > 0 && options[0].id) {
              ...
              foundMatch = true;
              metadata = {
                imageUrl: details.imageUrl || option.imageUrl || item.imageUrl || null,
                director: details.director || item.director || null,
                leadActor: details.leadActor || item.leadActor || null,
                releaseYear: details.releaseYear || option.releaseYear || item.releaseYear || null,
                rating: details.rating !== undefined ? details.rating : (option.rating !== undefined ? option.rating : (item.rating || null))
              };
            }
          }
          
          // Only update if we successfully matched and have changes
          if (foundMatch) {
            const itemRef = doc(db, 'users', profileUid, 'watchlist', item.id);
            await updateDoc(itemRef, metadata);
          }
        } catch (error) {
